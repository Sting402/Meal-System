...
import { getMenuList, getSubList } from '../../utils/utils'

// SimpleBar, for more info and examples you can check out https://github.com/Grsmto/simplebar/tree/master/packages/simplebar-vue
import SimpleBar from "simplebar";
import menu from "@/data/ridefoxmenu";
...
// let list = [1, 3, 4, 6, 8, 10, 11, 12, 21, 30]
let list = JSON.parse(localStorage.getItem('menu'))
//console.log(list);
const newList = getMenuList(list.Menus)
const newSubList = getSubList(list.Menus)
const menuList = []
//console.log(newList);
//console.log(newSubList);
newList.forEach(item => {
  menuList.push(menu.main[item])
})
//console.log(menuList);
//console.log(newSubList.length);
if (newSubList.length > 0) {
  for (let i = menuList[8].sub.length - 1; i >= 0; i--) {
    const subItem = menuList[8].sub[i];
    if (newSubList.includes(i + 1)) {
      menuList[8].sub.splice(i, 1);
    }
  }
  //console.log(menuList);
}
if (menuList) {
  navigation.value = menuList
}
  ...
  <nav id="sidebar" :class="{ 'with-mini-nav': withMiniNav }" aria-label="Main Navigation">
    <slot>
      ...
      <div class="content-header">
        <slot name="header">
          <!-- Logo -->
          <RouterLink :to="{ name: 'ridefox-dashboard' }" class="fw-semibold text-dual">
            <!-- <span class="smini-visible">
                                                    <i class="fa fa-circle-notch text-primary"></i>
                                                  </span> -->
            <!-- <span class="smini-hide fs-6 tracking-wider"> -->
            FOX Meal System
        ...
        <div>
          <slot name="header-extra">
            ...
            <div class="dropdown d-inline-block ms-1">
              ...
              <div class="dropdown-menu dropdown-menu-end dropdown-menu fs-sm smini-hide border-0" style="min-width: 8rem"
