...
watch(stateMeal, (newVal, oldVal) => {
    newVal.forEach((item, index) => {
        ...
        // if (item.meal[1].meal_Number) {
        //     console.log(123);
        //     item.meal[1].meal_Number = newVal
        // }
    })
})
...
const getinit = async () => {
    ...
    menu = list.MenusSelect.filter(item => {
        let data = [821, 822, 823, 824, 825, 826, 827].includes(item);//陣列的過濾跟字串不一樣
        //console.log(data);
        return data;
    })
    console.log(menu);
    ...
    try {
        ...
        let res = await $foreignInfo({
            meal_OrderCust: 4,       // 3:代訂產線 4:代訂倉庫 5:代訂新人 6:代訂外部人員 7:代訂外國同仁
            // order_Date: today,
            order_Date: today,
            meal_Order: "晚",
