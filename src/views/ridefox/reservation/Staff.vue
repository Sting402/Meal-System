...
let stateMeal = reactive([
  {
    meal: [
      { values: 0, title: "CY", dept_ID: "811_清" },
      { values: 0, title: "CY", dept_ID: "811_守" },
      { totals: 0, title: "CY" }
    ],
  },
  {
    meal: [
      { values: 0, title: "JK8", dept_ID: "811_清" },
      { values: 0, title: "JK8", dept_ID: "811_守" },
      { totals: 0, title: "JK8" }
    ],
  },
  {
    meal: [
      { values: 0, title: "JKS", dept_ID: "811_清" },
      { values: 0, title: "JKS", dept_ID: "811_守" },
      { totals: 0, title: "JKS" }
    ]
  },
  {
    meal: [
      { values: 0, title: "LJ", dept_ID: "811_清" },
      { values: 0, title: "LJ", dept_ID: "811_守" },
      { totals: 0, title: "LJ" }
    ],
  },
  {
    meal: [
      { values: 0, title: "TC", dept_ID: "811_清" },
      { values: 0, title: "TC", dept_ID: "811_守" },
      { totals: 0, title: "TC" }
    ],
  },
  {
    meal: [
      { values: 0, title: "TRI", dept_ID: "811_清" },
      { values: 0, title: "TRI", dept_ID: "811_守" },
      { totals: 0, title: "TRI" }
    ],
  },
  {
    meal: [
      { values: 0, title: "WQ", dept_ID: "811_清" },
      { values: 0, title: "WQ", dept_ID: "811_守" },
      { totals: 0, title: "WQ" }
    ],
  }
])
...
watch(stateMeal, (newVal, oldVal) => {
  newVal.forEach((item, index) => {
    if (item.meal[0].values !== 0 || item.meal[1].values !== 0) {
      item.meal[2].totals = item.meal[0].values + item.meal[1].values
    }
    else {
      item.meal[2].totals = 0
    }
  })
})
...
const getinit = async () => {
  ...
  menu = list.MenusSelect.filter(item => {
    let data = [841, 842, 843, 844, 845, 846, 847].includes(item);//陣列的過濾跟字串不一樣
    //console.log(data);
    return data;
  })
  console.log(menu);
  ...
  try {
    ...
    let res = await $foreignInfo({
      meal_OrderCust: 6,       // 3:代訂產線 4:代訂倉庫 5:代訂新人 6:代訂外部人員 7:代訂外國同仁
      // order_Date: today,
      order_Date: today,
      meal_Order: "晚",
      ...
      if (data.payload[0].siteDept) {
        data.payload[0].siteDept.forEach(item => {//循環葷食 先默認晚餐
          ...
          stateMeal.forEach(col => {
            ...
            if (col.meal[0].title == item.site_ID && col.meal[0].dept_ID == item.dept_ID) {
              //console.log(item.meal_Number);
              // console.log(col.meal[0].title);
              col.meal[0].values = item.meal_Number
            }
            if (col.meal[1].title == item.site_ID && col.meal[1].dept_ID == item.dept_ID) {
              //console.log(item.meal_Number);
              col.meal[1].values = item.meal_Number
            }
          })
        })
      }
...
const handleMealConfirm = async (date) => {
  ...
  for (const col of stateMeal) {//forEach只能同步 for of 可以異步
    ...
    let data = {
      "dept_ID": "811_清",
      "site_ID": col.meal[0].title,
      "meal_Number": col.meal[0].values
    }
    siteDept.push(data)
    let data1 = {
      "dept_ID": "811_守",
      "site_ID": col.meal[1].title,
      "meal_Number": col.meal[1].values
    }
    siteDept.push(data1)
  }
  try {
        ...
        <table class="table table-bordered table-striped table-vcenter">
          <thead>
            <tr>
              ...
              <th class="text-center" style="width: 10%">811_清</th>
              <th class="text-center" style="width: 10%">811_守</th>
          ...
          <tbody>
            <tr v-for="(item, key) in filteredCols" :key="item">
              ...
              <td class="text-center" style="width: 10%">
                <div id="app" style="width: 100%">
                  <input type="number" class="form-control w-100  text-center" min="0"
                    v-model="stateMeal[key].meal[2].totals" disabled="true" />
                </div>
              </td>
            </tr>
          </tbody>
          ...
          <tbody>
            <tr v-for="(item, key)  in filteredCols" :key="item">
              ...
              <td class="text-center" style="width: 10%">
                <div id="app" style="width: 100%">
                  <input type="number" class="form-control w-100  text-center" min="0"
                    v-model="stateVegetarian[key].meal[2].totals" disabled="true" />
                </div>
              </td>
            </tr>
          </tbody>
