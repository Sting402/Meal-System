...
function swalEditSuccess() {
    toast.fire("新增成功", "代訂產線成功", "success");
}
function swalInfo() {
...
const isMeal = ref(true) //v-if true是顯示
const isVegetarian = ref(true)
const isFilipino = ref(true)
const btnMeal = ref('確認')
const btnVegetarian = ref('確認')
const btnFilipino = ref('確認')
let date = new Date();
...
watch(stateMeal, (newVal, oldVal) => {
    newVal.forEach((item, index) => {//寫for循環會出現黃色抱錯
        if (item.meal[0].values !== 0 || item.meal[1].values !== 0 || item.meal[2].values !== 0 || item.meal[3].values !== 0 || item.meal[4].values !== 0 || item.meal[5].values !== 0) {
            item.meal[6].totals = item.meal[0].values + item.meal[1].values + item.meal[2].values + item.meal[3].values + item.meal[4].values + item.meal[5].values
        }
        else {
            item.meal[6].totals = 0
        }
    })
})
...
watch(stateFilipino, (newVal, oldVal) => {
    newVal.forEach((item, index) => {
        if (item.meal[0].values !== 0 || item.meal[1].values !== 0 || item.meal[2].values !== 0 || item.meal[3].values !== 0 || item.meal[4].values !== 0 || item.meal[5].values !== 0) {
            item.meal[6].totals = item.meal[0].values + item.meal[1].values + item.meal[2].values + item.meal[3].values + item.meal[4].values + item.meal[5].values
        }
...
watch(selectedMeal_Order, (newVal, oldVal) => {
    ...
    if (selectedDate > today) {
        ...
        isFilipino.value = true
        btnMeal.value = '確認'
        btnVegetarian.value = '確認'
        btnFilipino.value = '確認'
        ...
        if ((newVal === '早餐' && hour >= 6) ||
            ...
            isFilipino.value = false
            btnMeal.value = '查詢'
            btnVegetarian.value = '查詢'
            btnFilipino.value = '查詢'
            console.log(111);
        } else {
            isMeal.value = true;// 顯示
            isVegetarian.value = true
...
const getinit = async () => {
    ...
    menu = list.MenusSelect.filter(item => {
        let data = [811, 812, 813, 814, 815, 816, 817].includes(item);//陣列的過濾跟字串不一樣
        //console.log(data);
        return data;
    })
    //console.log(menu);
    ...
    // stateLight = stateLight.filter(col => {
    //     const title = col.meal[0].title;
    //     const isShow = cols.find(item => item.name === title)?.isShow;
    //     return isShow !== undefined && isShow === true;
    // });
    stateFilipino = stateFilipino.filter(col => {
        const title = col.meal[0].title;
        const isShow = cols.find(item => item.name === title)?.isShow;
        return isShow !== undefined && isShow === true;
    });
    try {
        ...
        let res = await $foreignInfo({
            meal_OrderCust: 3,       // 3:代訂產線 4:代訂倉庫 5:代訂新人 6:代訂外部人員 7:代訂外國同仁
            // order_Date: today,
            order_Date: today,
        ...
        if (data.success) {
            ...
            // if (data.payload[2].siteDept) {
            //     data.payload[2].siteDept.forEach(item => {//循環輕食 先默認晚餐
            //         stateVegetarian.forEach(col => {
            //             for (let i = 0; i < 6; i++) {
            //                 if (col.meal[i].title === item.site_ID && col.meal[i].dept_ID === item.dept_ID) {
            //                     col.meal[i].values = item.meal_Number;
            ...
            //     })
            // }
            if (data.payload[3].siteDept) {
                data.payload[3].siteDept.forEach(item => {//循環菲食 先默認晚餐
                    stateFilipino.forEach(col => {
                        for (let i = 0; i < 6; i++) {
                            if (col.meal[i].title === item.site_ID && col.meal[i].dept_ID === item.dept_ID) {
                                col.meal[i].values = item.meal_Number;
                            }
...
const handleConfirm = async (date) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            // if (data.payload[2].siteDept.length == 0) {
            //     stateLight.forEach(col => {
            //         for (let i = 0; i < 6; i++) {
            //             col.meal[i].values = 0
            //         }
            //     })
            // }
            if (data.payload[3].siteDept.length == 0) {
                stateFilipino.forEach(col => {
                    for (let i = 0; i < 6; i++) {
                        col.meal[i].values = 0
                    }
...
const handleMealConfirm = async (date) => {
    ...
    let siteDept = []
    console.log(stateMeal);
    const deptIds = ["833", "843", "844", "845", "846", "847"];
...
const handleVegetarianConfirm = async (date) => {
    ...
    console.log(stateVegetarian);
    const deptIds = ["833", "843", "844", "845", "846", "847"];
    for (const col of stateVegetarian) {//forEach只能同步 for of 可以異步
        for (let i = 0; i < 6; i++) {
            const data = {
                "dept_ID": deptIds[i],
                "site_ID": col.meal[i].title,
...
// const handleLightConfirm = async (date) => {
//     console.log(date);
//     let siteDept = []
//     console.log(stateLight);
//     const deptIds = ["833", "843", "844", "845", "846", "847"];
//     for (const col of stateLight) {//forEach只能同步 for of 可以異步
//         for (let i = 0; i < 6; i++) {
//             const data = {
//                 "dept_ID": deptIds[i],
//                 "site_ID": col.meal[i].title,
...
//             console.log(data);
//             data.payload.forEach(item => {
//                 if (item.order_Desc.startsWith("無供應商供應") || item.order_Desc === "無取餐地點") {
//                     let data = {
//                         'site_ID': item.site_ID,
//                         'dept_ID': item.dept_ID,
//                         'meal_Number': item.meal_Number,
...
// }
const handleFilipinoConfirm = async (date) => {
    console.log(date);
    let siteDept = []
    console.log(stateFilipino);
    const deptIds = ["833", "843", "844", "845", "846", "847"];
    for (const col of stateFilipino) {//forEach只能同步 for of 可以異步
        for (let i = 0; i < 6; i++) {
            const data = {
                "dept_ID": deptIds[i],
                "site_ID": col.meal[i].title,
...
<template>
    ...
    <div class="content">
                    ...
                    <tbody>
                        <tr v-for="(item, key) in filteredCols" :key="item">
                            ...
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateMeal[key].meal[6].totals" disabled="true" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <tbody>
                        <tr v-for="(item, key)  in filteredCols" :key="item">
                            ...
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateVegetarian[key].meal[6].totals" disabled="true" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
        ...
        <!-- <BaseBlock title="輕食">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <template #options>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <button class="btn btn-primary" type="button" @click=handleLightConfirm(flatPickrState.dateRange)>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                確認
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </button>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </template>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <div class="table-responsive">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <tbody>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    <tr v-for="(item, key) in filteredCols" :key="item">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[0].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[1].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[2].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[3].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[4].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[5].values" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td class="text-center" style="width: 10%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="app" style="width: 100%">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control w-100  text-center" min="0"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    v-model="stateLight[key].meal[6].totals" disabled="true" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    </tr>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                </tbody>
        ...
        <BaseBlock title="菲食">
            <template #options>
                <button class="btn btn-primary" type="button" :disabled="!isFilipino"
                    @click=handleFilipinoConfirm(flatPickrState.dateRange)>
                    {{ btnFilipino }}
                </button>
            </template>
            <div class="table-responsive">
                    ...
                    <tbody>
                        <tr v-for="(item, key) in filteredCols" :key="item">
                            ...
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[0].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[1].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[2].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[3].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[4].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[5].values" />
                                </div>
                            </td>
                            <td class="text-center" style="width: 10%">
                                <div id="app" style="width: 100%">
                                    <input type="number" class="form-control w-100  text-center" min="0"
                                        v-model="stateFilipino[key].meal[6].totals" disabled="true" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
